...
class Item {
    constructor({id, name, age, dob, retired, date, version}) {
        this.id = id;
        this.name = name;
        this.age = age;
        this.dob = dob
...
const items = [];
const dateOfDob = new Date(1995, 8, 1, 12)

for (let i = 0; i < 3; i++) {
    dateOfDob.setDate(i + 1);
    items.push(new Item({
        id: `${i}`,
        name: `actor ${i}`,
        age: 1 + i,
        dob: dateOfDob.toDateString(),
...
router.get('/item', ctx => {
    ...
    const sortedItems = items
        .filter(item => name ? item.name.indexOf(name) !== -1 : true)
...
const createItem = async (ctx) => {
    ...
    if (!item.age) {
        ctx.response.body = {issue: [{error: 'Age is missing'}]};
        ctx.response.status = 400; //  BAD REQUEST
        return;
    }
    if (!item.dob) {
        ctx.response.body = {issue: [{error: 'Date of birth is missing'}]};
        ctx.response.status = 400; //  BAD REQUEST
        return;
    }
    if (!item.retired) {
        ctx.response.body = {issue: [{error: 'Is a retired actor? is missing'}]};
        ctx.response.status = 400; //  BAD REQUEST
        return;
    }
...
setInterval(() => {
    ...
    let currentAge = `${parseInt(lastId) + 1}`;
    const currentDateOfAnActor = new Date(1995, 8, 1, 12);
    currentDateOfAnActor.setDate(currentDateOfAnActor.getDate() + parseInt(lastId));
    const item = new Item({
        id: lastId,
        name: `actor ${lastId}`,
        age: currentAge,
        dob: currentDateOfAnActor.toDateString(),
        retired: false.toString(),
        date: lastUpdated,
        version: 1
    });
