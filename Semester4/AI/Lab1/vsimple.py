...
class Environment():
    ...
    def image(self, columnour=BLUE, background=WHITE):
...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        ...
        if y < 19 and detectedMap.surface[x][y + 1] == 0 and self.visited[x][y + 1] == 0:
            self.stack.append([x, y + 1])
            newMove += 1
            self.previous[(x, y + 1)] = (x, y)

        elif y > 0 and detectedMap.surface[x][y - 1] == 0 and self.visited[x][y - 1] == 0:
            self.stack.append([x, y - 1])
            newMove += 1
            self.previous[(x, y - 1)] = (x, y)

        elif x < 19 and detectedMap.surface[x + 1][y] == 0 and self.visited[x + 1][y] == 0:
            self.stack.append([x + 1, y])
            newMove += 1
            self.previous[(x + 1, y)] = (x, y)
        elif x > 0 and detectedMap.surface[x - 1][y] == 0 and self.visited[x - 1][y] == 0:
            self.stack.append([x - 1, y])
            newMove += 1
            self.previous[(x - 1, y)] = (x, y)
        ...
        if newMove == 0:
            if x == self.startX and y == self.startY:
                ...
                self.stack.append([self.previous[(x, y)][0], self.previous[(x, y)][1]])
